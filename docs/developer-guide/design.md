...
5. **Reviewers converge over time.** The escape-to-improvement loop is where the system gets better. A reviewer that's "good enough" and then improved from real escapes beats one you tried to design perfectly from the start. This is again a governance story: the human team is responsible for the reviewers, and they adjust them based on the escapes that happen in production; the reviewers will never get everything perfectly correct, but over time humans can steer them in the right direction from real feedback from the codebase and from agents merging code.
