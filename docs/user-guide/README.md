...
agent looping against it -- because Bastion is built so both see the same review
through whatever surface is natural to each.
...
7. **[Governance](./governance.md)** -- keeping humans at the policy layer:
   CODEOWNERS, branch protection, the escape-to-improvement loop, and what
...
Bastion is experimental but no longer hollow. The routing, runner, verdict
aggregation, and on-disk run store are real and tested; the Claude Code and Codex
backends execute reviewers for real. Some schema fields are accepted but not yet
provisioned -- those are called out explicitly where they appear in
