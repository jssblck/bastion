...
description: Apply Alexis King's "Parse, don't validate" design rule. Use when adding or reviewing Rust boundary code for TOML, JSON, CLI/env input, deserialized config, or any function that checks input validity before deeper processing.
---
...
1. Locate the boundary where untrusted or weakly typed data enters: file load, serde decode, CLI/env parsing, generated data, user edits, or subprocess output.
...
3. Write a parser/refinement constructor that consumes the weaker input and returns the stronger type, such as `parse_config(raw) -> Result<Config>`.
...
- Raw serde structs may exist at the boundary. Name the checked form for what it proves, such as `CheckedConfig` or a domain-specific invariant name.
...
Clippy's `unnecessary_wraps` lint flags functions that claim fallibility without needing it. A lint or nudge rule can additionally block new functions named `validate*` or `parse*` that return `Result<()>` instead of the parsed type.
